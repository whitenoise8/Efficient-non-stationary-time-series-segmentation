...
function get_MCMC_objects(s; periodogram = false)
  ...
  for j in 1:(n_CP_sample[1]+1)
    ...
    β_var_aux = inv(I(2*n_freq_sample[j, 1]+2)/(σ_β^2) + (X_aux'*X_aux)/(σ_sample[j, 1]^2))
...
function within_move_non_stationary(t, n_CP_sample, β_sample, ω_sample,
  ...
  β_var_prop_1 = inv(I(2*n_freq_prop_1 + 2)/(σ_β^2) + (X_prop_1'*X_prop_1)/(σ_current_1^2))
  ...
  β_var_prop_2 = inv(I(2*n_freq_prop_2 + 2)/(σ_β^2) + (X_prop_2'*X_prop_2)/(σ_current_2^2))
  ...
  β_var_curr_1 = inv(I(2*n_freq_curr_1 + 2)/(σ_β^2) + (X_curr_1'*X_curr_1)/(σ_current_1^2))
  ...
  β_var_curr_2 = inv(I(2*n_freq_curr_2 + 2)/(σ_β^2) + (X_curr_2'*X_curr_2)/(σ_current_2^2))
...
function birth_move_non_stationary(t, n_CP_sample, β_sample, ω_sample,
  ...
  X_prop_1 = get_X(ω_star_1, a, b)
  β_var_prop_1 = inv(I(2*n_freq_prop_1 + 2)/(σ_β^2) + (X_prop_1'*X_prop_1)/(σ_proposed_1^2))
  β_var_prop_1 = 0.5*(β_var_prop_1 + β_var_prop_1')
  ...
  X_prop_2 = get_X(ω_star_2, a, b)
  β_var_prop_2 = inv(I(2*n_freq_prop_2 + 2)/(σ_β^2) + (X_prop_2'*X_prop_2)/(σ_proposed_2^2))
  β_var_prop_2 = 0.5*(β_var_prop_2 + β_var_prop_2')
  ...
  β_var_curr = inv(I(2*n_freq_current+2)/(σ_β^2) + (X_curr'*X_curr)/(σ_current^2))
  ...
  # - beta
  log_prior_β_1_proposed = log(pdf(MvNormal(zeros(length(β_proposed_1)), (σ_β^2)*I(length(β_proposed_1))),
                            β_proposed_1)[1])

  log_prior_β_2_proposed = log(pdf(MvNormal(zeros(length(β_proposed_2)), (σ_β^2)*I(length(β_proposed_2))),
                            β_proposed_2)[1])
  log_prior_β_proposed = log_prior_β_1_proposed + log_prior_β_2_proposed

  log_prior_β_current = log(pdf(MvNormal(zeros(length(β_current)), (σ_β^2)*I(length(β_current))),
                           β_current)[1])
...
function death_move_non_stationary(t, n_CP_sample, β_sample, ω_sample,
  ...
  β_var_prop = inv(I(2*n_freq_star + 2)/(σ_β^2) + (X_prop'*X_prop)/(σ_star^2))
  ...
  β_var_curr_1 = inv(I(2*n_freq_current_1 + 2)/(σ_β^2) + (X_curr_1'*X_curr_1)/(σ_current_1^2))
  β_var_curr_1 = 0.5*(β_var_curr_1 + β_var_curr_1')
  β_mean_curr_1 = β_var_curr_1*((X_curr_1'*y_curr_1)/(σ_current_1^2))
  ...
  β_var_curr_2 = inv(I(2*n_freq_current_2 + 2)/(σ_β^2) + (X_curr_2'*X_curr_2)/(σ_current_2^2))
  β_var_curr_2 = 0.5*(β_var_curr_2 + β_var_curr_2')
  β_mean_curr_2 = β_var_curr_2*((X_curr_2'*y_curr_2)/(σ_current_2^2))
  ...
  # - β
  log_prior_β_proposed = log(pdf(MvNormal(zeros(length(β_proposed_star)), (σ_β^2)*I(length(β_proposed_star))),
                            β_proposed_star)[1])

  log_prior_β_1_current = log(pdf(MvNormal(zeros(length(β_current_1)), (σ_β^2)*I(length(β_current_1))),
                            β_current_1)[1])
  log_prior_β_2_current = log(pdf(MvNormal(zeros(length(β_current_2)), (σ_β^2)*I(length(β_current_2))),
                            β_current_2)[1])
