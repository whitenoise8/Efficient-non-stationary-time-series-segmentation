...
λ_S = 2  # poisson prior parameter, i.e n of freq ∼ Poisson(λ_S)
...
# -- Change-Point model:
λ_NS = 1 # poisson prior parameter, i.e n of cp ∼ Poisson(λ_NS)
...
lambdas_NS = [0.1, 1, 10] # poisson prior parameter, i.e n of cp ∼ Poisson(λ_NS)
for l_setting in 1:3
  λ_NS = lambdas_NS[l_setting] # poisson prior parameter, i.e n of cp ∼ Poisson(λ_NS)

  # ------------------------------------------------------ #
  # ----------------- Simulation Study  ------------------ #
  # ------------------------------------------------------ #
  collect_cp = zeros(Nsim, n_CP_max)
  collect_s = zeros(n_obs, Nsim)
  collect_time = zeros(Nsim, 1)

  for sim_index in 1:Nsim
    fname = join(["data_sim/setting_n",n_obs,"_ncp",ncp_true,"/data",sim_index,".txt"])
    ...
    print("Iteration: ",sim_index," of lambda: ",λ_NS,"\n")

  end

  writedlm(join(["res_julia/cp_setting_n",n_obs,"_ncp",ncp_true,"_lambda_setting",l_setting,".txt"]),  collect_cp, ',')
  writedlm(join(["res_julia/time_setting_n",n_obs,"_ncp",ncp_true,"_lambda_setting",l_setting,".txt"]),  collect_time, ',')
  writedlm(join(["res_julia/signal_setting_n",n_obs,"_ncp",ncp_true,"_lambda_setting",l_setting,".txt"]),  collect_s, ',')
