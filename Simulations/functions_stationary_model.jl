...
function within_move_stationary(m_current, β_current, ω_current,
  ...
  β_var_post = inv(I(2*m_current+2)/(σ_β^2) + (X_post'*X_post)/(σ^2))
...
function birth_move_stationary(m_current, β_current, ω_current,
  ...
  β_var_prop = inv(I(2*m_proposed+2)/(σ_β^2) + (X_prop'*X_prop)/(σ^2))
  ...
  β_var_curr = inv(I(2*m_current+2)/(σ_β^2) + (X_curr'*X_curr )/(σ^2))
  ...
  log_β_prior_ratio = (log.(pdf(MvNormal(zeros(2*m_proposed+2), (σ_β^2)*I(2*m_proposed+2)) , β_proposed))[1]) -
                       log.(pdf(MvNormal(zeros(2*m_current+2), (σ_β^2)*I(2*m_current+2)), β_current))
